...
- 给 `working-set` 增加 `primary_evidence_gap` / `next_minimal_action` / `verification_focus`
...
- 为其新增 `primary_evidence_gap`、`next_minimal_action`、`verification_focus`
- 更新相关命令模板与宿主提示词
...
- `primary_evidence_gap` 应由 planner 决定，还是由 working-set 在压缩阶段二次推断？
