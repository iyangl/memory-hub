...
- README.md — 项目说明：`.memory/` 统一目录、lane 分层、`project-memory` / `memory-admin` / unified MCP 用法、discover / session-extract 与自测方式
...
- lib/cli.py — CLI 命令分发入口，包含 discover/review/rollback/session-extract
- lib/discovery_cli.py — decision discovery CLI：读取当前 diff 和可选摘要，输出结构化候选
