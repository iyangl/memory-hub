...
> 说明：本文件主要描述 `.memory/` 文件型知识库的设计。durable memory v1 的当前基线已经迁移到 `.memoryhub/ + MCP + CLI review`，请以 `README.md`、`CLAUDE.md` 和 `.archive/plans/2026-03-10-memory-hub-v1-*.md` 为准。
## 定位
...
当时的设计假设是：Skill（AI 直接执行的提示词流程），不是 MCP 服务；文件型知识库层面无服务进程、无数据库、无协议层。当前 durable memory v1 已新增 `.memoryhub/ + MCP + CLI review` 控制面，但这不改变本节对早期 `.memory/` 文件型工作流的历史说明。
